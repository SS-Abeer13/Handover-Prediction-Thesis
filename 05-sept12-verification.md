...
New code: `src/hoproj/data/signalling.py`, `configs/adapters/xcal_signalling.yaml`,
...
simultaneously, including two aggressive ones.**
...
(151) -> windows -> four models. **R4 gate cleared.**

Prevalence is 8.6/16.1/22.6/31.4%.
...
training drives and 24 minutes of data that is expected, not a defect.
...
1. **RQ6 is back on the table**, conditional on neighbour coverage. At 33% it is
   thin; 60%+ would make candidate ranking properly testable.
2. **HOF/RLF moves from exploratory toward primary** if the re-establishment rate
   holds — 64 in 24 min is a lot of events.
3. **The A3 rule baseline stops being a guess.** It can be parameterised with the
   network's actual offset, hysteresis and TTT.
4. **QoE is still the gap.** RTT and packet loss are empty in both XCAL captures.
   Client-side active tests (iPerf / ping) during the drive are the fix.
