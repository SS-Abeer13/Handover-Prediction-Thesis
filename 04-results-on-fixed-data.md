# Results on the fixed data — supersedes doc 02
...
Everything in doc 02 is void — it was measured on the synthetic file. On the
device those outputs are parked in `reports_SYNTHETIC_RUN_do_not_use/` and
`artifacts_SYNTHETIC_RUN_do_not_use/`.
...
margin is 1.25x rather than 5.5x, with a slightly lower detection rate. Event
detection is 23-26% at 2 s against 70% before. **Do not repeat the "GRU gives the
same warnings for a fifth of the false alarms" claim.**
...
This is the strongest result in the run, and it survived the data change — itself
evidence that it is about overlapping windows rather than any particular dataset.
...
**Calibration is poor on the external route** — ECE 0.17-0.28, far worse than the
~0.10 seen in development. Temperature scaling fitted on development calibration
drives does not transfer to the new corridor. Genuine, reportable, and new: the
synthetic file hid it.
...
set at every horizon** — time since last handover, serving dwell,
return-to-source. That makes sense in a network with 38% ping-pong. QoE still
adds nothing.
...
2. **Lead with RQ3.** GRU AUPRC doubling under random splitting against
   ...
   different datasets.
3. **Report that sequence models lose to LightGBM here.** A negative result the
   proposal explicitly commits to reporting.
4. **The external-route calibration failure is a new finding** worth its own
   subsection — exactly the "confidently wrong under distribution shift" problem
   the thesis set out to study.
5. **Provenance is still unresolved.** The fixed file passes the physical checks,
   but nobody has said where it comes from. Establish that before submission.
