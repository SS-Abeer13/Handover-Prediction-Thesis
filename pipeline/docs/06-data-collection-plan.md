...
Both now run through the pipeline. Plain-language version of the decisions.
...
Same pattern twice, two days apart. This is real network behaviour, not a
logging artefact.
...
stays better for Y milliseconds.* X is the "offset", Y is the
"time-to-trigger". The logs contain the values the operator is actually using:
...
Negative offsets plus a 160 ms trigger explain the 2.6-second spacing. The
operator is running all of these profiles at once across its carriers.
...
Consequence: the shortest horizon the data can label is 1 second, and the
0.5 s horizon in the proposal stays impossible.
...
62–63% of the phone's measurement reports contain **no neighbour at all** (they
are A1/A2 serving-cell-only reports). Only ~30% of grid samples end up with a
neighbour value.

Consequence: candidate-cell ranking (RQ6) cannot be trained properly, and the
serving-to-neighbour gap — the strongest single predictor — is missing on 70%
of samples.
...
server for latency, an iPerf or large HTTP download for throughput. Nothing
about the phone or XCAL needs to change; this is a second program running on
the laptop.

### 4. Volume — blocking everything

Two drives, ~70 minutes total. The micro-pilot put the GRU *below* the
prevalence floor at every horizon, which is what 6 training drives buys.

**Action:** 30+ drives with signalling before any model claim. The Sept 6–8
curated dataset (117 drives) stays the modelling set meanwhile.
...
That is a strong angle: *predicting which handovers are unnecessary, in a
network demonstrably configured to over-handover.* It uses the ping-pong labels
the pipeline already produces and needs no new capability — only more drives.
