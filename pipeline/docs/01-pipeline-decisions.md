...
Forecasting*. Code delivered as `ho-pipeline.zip`; target home is
`D:\Handover Thesis\pipeline`.
...
| `HANDOVER_LOG_1.csv` | 2,300 events | authoritative handover labels, with corridor names |
...
Prevalence at 2 s is 6 %, so AUPRC 0.58 is a ~9× lift. Leakage study already
shows random-row splitting inflating AUPRC by 1–7 % — real but modest, which is
itself a reportable finding. Sequence models separate from LightGBM mainly on
**false alarms per hour** (6 vs 37), not on AUPRC — that is the interesting
story for the paper. All numbers are from 3–4 epoch runs; treat as wiring
...
Full epoch budgets on your GPU box. Then: tune window length inside the dev
domain only, and decide whether the QoE degradation definition (percentile-based,
train-fitted) matches what you want to claim.
