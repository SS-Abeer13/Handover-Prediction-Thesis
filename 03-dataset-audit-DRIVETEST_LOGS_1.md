...
| file | verdict |
|---|---|
| DRIVETEST_LOGS_1.csv (original) | **SYNTHETIC** |
...
| test 13 sept (XCAL + signalling) | consistent with measured data |
...
### 1. HANDOVER_LOG_1.csv is stale — blocking (since fixed)

It was not regenerated with the sample file, and describes a different network:
...
Zero shared names. The audit now runs an `event/sample pairing` check first and
refuses to interpret the others when it fails.

**Action taken:** regenerated with `python -m hoproj.data.event_log` from the
same export -> `HANDOVER_LOG_REGENERATED.csv`, 100% self-consistent. See doc 04.

### 2. The neighbour columns include the serving cell
...
candidate slots carry no information. Probably an export convention (some tools
list the strongest *detected* cell, serving included) rather than an error, but
it halves the effective candidate count and is the main reason
`target_is_neighbour` sits at 38% rather than 60%+.

**Action:** confirm the convention in XCAL. The audit now excludes the serving
cell from candidates; if confirmed, the feature builder should shift the
candidate slots too.
...
5. `switch_physics` failed on the real captures because event-triggered
