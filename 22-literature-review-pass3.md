...
```
bibliography        88 entries  ->  108        (93 verified=full, 15 partial)
...
And a separate defect, found by an automated sweep of the file rather than by
reading it: **four pass-1 entries carry no `author` field at all** —
`crossoperator2026outage`, `intime2025cho`, `residuallstm2024mobility` and
`preho2026leo`. A fifth, `survey2024mlhandover`, had the same defect and was
fixed in §6. These four must be completed or dropped before submission; none of
them is cited for a result.
...
## 8. What this changed in the manuscript

All of it is applied; this section is a record, not a to-do list.

| where | what changed |
|---|---|
| **§7** (survival formulation) | opens by stating the formulation is borrowed — Wiegrebe et al., Thorsen-Meyer et al. — and that what is new is where it is pointed. The C-index departure is named and defended. |
| **§11.3** (event-level metrics) | Wagner et al. added as the reason event-level costs are reported *alongside* row-level AUPRC rather than instead of it. |
| **§12** (risk control) | now opens with Cohen et al. and Simeone et al. **before** the method, and states the narrowed claim in its first paragraph. |
| **§13.2 / §22** (point process) | Laub et al. for the model family and the branching ratio; Price-Williams and Heard for why the residual test is the local norm. |
| **§19** (transfer) | Sun and Saenko cited at the method; Ismail Fawaz et al. added as the reason the CORAL null is credible rather than an implementation failure. |
| **§24** (contributions) | C3 rewritten around the exchangeability unit with the prior art named in the row; C13 rewritten for 22 papers with the five exceptions listed. |
| **§25 / §25.2** | rewritten in pass 3; §25.2 is new. |
| **§27** (limitations) | three new limitations: the two narrowed claims, and the four `verified=partial` author lists. |
| **18v2** | C3 and C13 carry the same narrowings, with the history. |
| **doc 16** | status banner: counts superseded, findings stand. |
| **deck** | three new literature slides; the conformal slide now names the prior art on the slide itself. |

Nothing in the results changed. Six sections got a better-defended argument, two
claims got smaller, and one got caught.

### 8.1 One thing this pass caught by accident

Wiring the citations in meant looking for the section that documents the
deep-ensemble arm, so that Lakshminarayanan et al. could be cited from it.
**There is no such section.** `stage03_uncertainty` trains a five-member deep
ensemble, it ran on the old curated dataset, and it was never re-run on the
pooled XCAL captures — there is no ensemble row anywhere in
`reports_xcal/tables/`. Meanwhile the `OURS` row of `protocol_audit.csv` had
been listing "ensembles" among our uncertainty methods, against competitors
scored on exactly that column.

It is a small overclaim and it has been corrected: the audit row now reads
temperature scaling, isotonic, monotone projection, split conformal and
conformal risk control, and MASTER §27 records that the ensemble arm is code
without a result and must be re-run or dropped.

Worth noting how it was found. This is the second claim in this project to
outlive the thing it described — the first was a figure-generating stage cited
in §26 that does not exist — and **both were caught by trying to cite them**,
not by reading. A claim survives proofreading because it reads well. It does not
survive someone going to look for its evidence.
