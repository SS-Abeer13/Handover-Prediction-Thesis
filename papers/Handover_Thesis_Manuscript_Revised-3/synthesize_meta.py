...
out_dir = Path("papers/Handover_Thesis_Manuscript_Revised-3")

review_alfa = (out_dir / "review_alfa.md").read_text(encoding="utf-8")
review_bravo = (out_dir / "review_bravo.md").read_text(encoding="utf-8")
review_charlie = (out_dir / "review_charlie.md").read_text(encoding="utf-8")

meta_text = """# Synthesis Meta-Review: Leakage-Audited Multi-Horizon Handover Forecasting from LTE Drive-Test Signalling
...
```json
{
  "concerns": [
    {
      "concern": "Pseudo-replication and anti-conservative confidence intervals from within-campaign block bootstrapping",
      "alfa": true,
      "bravo": true,
      "charlie": false
    },
    {
      "concern": "Severely underpowered sample size (N=4 sessions) and uninformative cross-campaign inference",
      "alfa": true,
      "bravo": true,
      "charlie": true
    },
    {
      "concern": "Multiplicity, post-hoc pipeline tuning, and unadjusted multi-hypothesis comparisons",
      "alfa": true,
      "bravo": true,
      "charlie": true
    },
    {
      "concern": "Breakdown of distribution-free conformal risk guarantees under cross-campaign shift and grid discretization",
      "alfa": true,
      "bravo": true,
      "charlie": true
    },
    {
      "concern": "Operational unviability of proactive target-cell preparation under 1-Hz telemetry (179 false alarms/hr, 0.47s lead time)",
      "alfa": false,
      "bravo": true,
      "charlie": true
    },
    {
      "concern": "Survival formulation provides no empirical advantage over simple monotonic projections on independent heads",
      "alfa": true,
      "bravo": false,
      "charlie": true
    },
    {
      "concern": "Duplicated and corrupted equation tags, malformed expressions, and dead cross-references",
      "alfa": true,
      "bravo": true,
      "charlie": true
    },
    {
      "concern": "Contradictory calibration error statements between pooled and per-campaign metrics, and missing Appendix A data",
      "alfa": false,
      "bravo": true,
      "charlie": false
    },
    {
      "concern": "Absence of qualitative trace-level error analysis and repurposing of Chapter 6 as an administrative OBE checklist",
      "alfa": false,
      "bravo": false,
      "charlie": true
    },
    {
      "concern": "Inconsistent reporting of branching ratio from a rejected, misspecified Hawkes process",
      "alfa": false,
      "bravo": true,
      "charlie": false
    },
    {
      "concern": "Proprietary binary XCAL diagnostic logs restrict independent verification of ASN.1 parser timeline",
      "alfa": false,
      "bravo": false,
      "charlie": true
    },
    {
      "concern": "Selection bias and base-rate confounding in quiet vs. burst row stratification",
      "alfa": true,
      "bravo": false,
      "charlie": false
    },
    {
      "concern": "Confounding RF propagation boundaries with intersection kinematics and timer cadence",
      "alfa": false,
      "bravo": false,
      "charlie": true
    }
...
# Parse JSON block
json_match = re.search(r"```json\s*(.*?)\s*```", meta_text, re.DOTALL)
if json_match:
    json_data = json.loads(json_match.group(1))
    concerns = json_data["concerns"]
    df_concerns = pd.DataFrame(concerns)
    csv_path = out_dir / "concerns_table.csv"
    df_concerns.to_csv(csv_path, index=False)
    print(f"Wrote {len(df_concerns)} concerns to {csv_path}")

# Strip JSON block from saved meta_review.md
clean_meta = re.sub(r"## CONCERNS_TABLE_DATA\s*```json.*?```", "", meta_text, flags=re.DOTALL).strip() + "\n"
(out_dir / "meta_review.md").write_text(clean_meta, encoding="utf-8")
print(f"Wrote clean meta_review.md ({len(clean_meta)} chars)")
...
results_bundle = {
    "individual_reviews": {
        "alfa": review_alfa,
        "bravo": review_bravo,
        "charlie": review_charlie
    },
    "meta_review": clean_meta,
    "reviewer_mapping": {
        "alfa": "gemini-subagent-alfa",
        "bravo": "gemini-subagent-bravo",
        "charlie": "gemini-subagent-charlie (alignment-forum-critic)"
    }
}
(out_dir / "results.json").write_text(json.dumps(results_bundle, indent=2), encoding="utf-8")
print("Wrote results.json successfully")
